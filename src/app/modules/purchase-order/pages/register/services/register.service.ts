...
export class RegisterService {
  ...
  public findbySupplyCode(supplyCode: string): RqPoSelectedItem {
    const products: RqPoSelectedItem[] = this.currentProducts.value ?? [];
    return products.find(e => e.supplyCode === supplyCode);
  }

  public updateBySuplyCode(item: RqPoSelectedItem): void {
    const products: RqPoSelectedItem[] = this.currentProducts.value ?? [];
    const productFounded: RqPoSelectedItem = products.find(e => e.supplyCode === item.supplyCode);    
    if (productFounded) {
      const index: number = products.indexOf(productFounded);
