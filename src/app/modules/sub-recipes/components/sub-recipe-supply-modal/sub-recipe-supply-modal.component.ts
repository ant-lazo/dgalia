...
@Component({
  selector: 'app-sub-recipe-supply-modal',
  templateUrl: './sub-recipe-supply-modal.component.html',
  styleUrls: ['./sub-recipe-supply-modal.component.scss']
})
export class SubRecipeSupplyModalComponent implements OnInit {

  public filteredOptions: Observable<Supply[]>;
  public supplyList: Supply[] = [];
  public searchParam = new FormControl();
  public selectedList: SubRecipeSelectedSupply[] = [];
  ...
  constructor(
    private _dialogRef: MatDialogRef<SubRecipeSupplyModalComponent>,
  ...
  public setSupply(event: MatAutocompleteSelectedEvent): void {
    ...
    if (!exist) {
      const newItem = new SubRecipeSelectedSupply(
        founded.id,
        founded.code,
        founded.name,
